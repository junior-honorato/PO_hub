...
export default function ProjectOverview({ projectId, onBack, onSelectDemand }) {
  ...
  const [activeTab, setActiveTab] = useState('operational'); // 'operational' or 'report'
  ...
  // Lógica de agrupamento por Epic/Eixo
  const epics = [];
  const epicMap = {};
  ...
  demands.forEach(d => {
    ...
    if (isEpicType || isParentOfSomeone) {
      epics.push(d);
      epicMap[d.externalId] = [];
    }
  });

  demands.forEach(d => {
    if (epicMap[d.externalId] !== undefined) {
      return;
    }
    const pId = d.parentId;
    if (pId && epicMap[pId] !== undefined) {
      epicMap[pId].push(d);
    } else {

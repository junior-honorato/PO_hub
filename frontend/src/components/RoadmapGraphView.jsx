...
export default function RoadmapGraphView({ demands, onSelectDemand }) {
  ...
  useEffect(() => {
    ...
    // 1. Create nodes
    const initialNodes = demands.map((d) => ({
      id: d.externalId,
      type: 'demandNode',
      data: {
    ...
    demands.forEach((d) => {
      ...
      if (d.parentId) {
        // Verify parent exists in demands
        const parentExists = demands.some((p) => p.externalId === d.parentId);
        if (parentExists) {
          initialEdges.push({
      ...
      if (d.blockers && Array.isArray(d.blockers)) {
        d.blockers.forEach((blockerId) => {
          const blockerExists = demands.some((b) => b.externalId === blockerId);
          if (blockerExists) {
            initialEdges.push({

...
20. **Filtro por Categoria Unificada no Report Executivo:** Nova lógica de exibição baseada na Categoria Unificada mapeada das demandas. Oculta demandas nas categorias inativas (`Backlog`, `Em Refinamento`, `Entregue`) exceto se possuírem impedimentos/riscos preenchidos (`blocker_notes`), e exibe demandas ativas (`Desenvolvimento` e `Homologação`).

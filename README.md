...
21. **Exportação Direta para PowerPoint (.pptx):** Geração dinâmica de slides do PowerPoint em formato widescreen 16:9 usando a biblioteca `PptxGenJS` (servida localmente). O arquivo exportado conta com um tema Dark Mode premium (fundo escuro `#0B0F19` e realces esmeralda `#34D399`) e numeração de slides. Para evitar erros de corrupção ou necessidade de reparo no PowerPoint, o sistema implementa paginação manual baseada na contagem real de linhas de texto por demanda, garantindo que as linhas da tabela permaneçam sempre íntegras no mesmo slide. Além disso, todos os elementos visuais do cabeçalho (título, metadados de data/sponsor, retângulo de fundo e linhas divisórias) são desenhados diretamente como formas e caixas de texto editáveis no slide, permitindo que o usuário os selecione, mova ou altere livremente no PowerPoint.
